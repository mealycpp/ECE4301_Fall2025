...
#1)Commands to run in terminal to run to get all necessary libraries
sudo apt update
...
#2)Install rust on PI if not already installed
curl -fsSL https://sh.rustup.rs | sh -s -- -y
source "$HOME/.cargo/env"
rustup default stable

#3)Test the camera functionality no receive/send video feed just testing
cam -l
...
#4)Need to run these 2 after to reset and kill current cam operation
pkill -f 'gst-launch-1.0.*libcamerasrc'
pkill -f 'rpi-secure-stream.*--payload=video'


#5)Open which encryption method you want from Files(RSA or ECDH)


#6)For self streaming test:
...
#7)Need to run these 2 after to reset and kill current cam operation
pkill -f 'gst-launch-1.0.*libcamerasrc'
pkill -f 'rpi-secure-stream.*--payload=video'

#8)To set addresses of each PI’s
...
#9)For direct send/receive mode(Only 2 PIs)
#Receive mode
...
#11)run it again in a another terminal window in each pi with the roles and addresses reversed to send video from the second pi to the first pi.

#12)Need to run these 2 after to reset and kill current cam operation
pkill -f 'gst-launch-1.0.*libcamerasrc'
pkill -f 'rpi-secure-stream.*--payload=video'
...
#1)Open up the 3node Folder and enter its directory in terminal

#2)Run these 1 of 3 command from below in the terminal of each Pi to set addresses to each
sudo nmcli con add type ethernet ifname eth0 con-name p2p-eth \
  ipv4.method manual ipv4.addresses 10.42.0.1/30 ipv4.never-default yes \
  ipv6.method ignore autoconnect yes
sudo nmcli con up p2p-eth
...
#3)Type this in terminal to see ip ethernet address
ip addr

#4)Build all 3 pi’s files
cargo build --release

#5)Run this command in each terminal
